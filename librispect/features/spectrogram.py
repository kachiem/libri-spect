...
def get_params(hparams):
    ...
    hop_length = int(hparams['frame_shift_ms'] /
                     1000 * hparams['sample_rate'])
    win_length = int(hparams['frame_length_ms'] /
                     1000 * hparams['sample_rate'])
...
def stft_to_wav(stft, hparams):
    ...
    return librosa.istft(_i_phase_and_magnitude(stft), hop_length=hop_length, win_length=win_length)
...
def wav_to_stft(data, hparams, pre=False):
    ...
    stft_imag = librosa.stft(data, n_fft=n_fft, hop_length=hop_length,
...
def normalize(stft, hparams):
    ...
    return np.clip((stft - hparams['min_level_db']) /
    ...
    def batch_iter(self, path_list, batch_size, shuffle=True):
        while True:
            for spect, stft in self.spect_iter(path_list):
                ...
                stft_sliced = slicing_window(
                    stft, self.window_size, self.step_size)
                if shuffle:
                ...
                for spect_batch, stft_batch in zip(np.array_split(spect_sliced, num_batches),
