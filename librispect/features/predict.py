...
class spect_predict_maker:
    def __init__(self, hparams, terms=4, predict_terms=4, step_size=1):
        ...
        self.spect_maker = spect_maker(hparams, window_size=self.window_size, step_size=step_size)
    ...
    def batch_iter(self, path_list, batch_size):
        while True:
            for spect, _ in self.spect_maker.spect_iter(path_list):
                ...
                for x_term_batch, x_pterm_batch, neg_term_batch, neg_pterm_batch in zip(*[np.array_split(sliced, num_batches) for sliced in [x_terms, x_pterms, neg_terms, neg_pterms]]):
                    # keras likes time to be dimension 1
                    term_batch = np.transpose(np.concatenate((x_term_batch, neg_term_batch)), (0, 2, 1))
                    pterm_batch = np.transpose(np.concatenate((x_pterm_batch, neg_pterm_batch)), (0, 2, 1))
                    labels = np.concatenate((np.ones(x_term_batch.shape[0]), np.zeros(neg_term_batch.shape[0])))
                    idxs = np.random.permutation(labels.shape[0])
                    yield [term_batch[idxs, ...], pterm_batch[idxs, ...]], labels[idxs, ...]